...
docker run \
  ...
  sh -c "poetry run update_db --database-file /data/sorry_database.json --stats-file /data/update_database_stats.json --log-file \"/data/logs/${LOG_FILE}\" --log-level DEBUG \
    && poetry run deduplicate_db --database-file /data/sorry_database.json --results-file /data/deduplicated_sorries.json --log-file \"/data/logs/${LOG_FILE}\" --log-level DEBUG"
